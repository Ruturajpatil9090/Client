...
const ApiDataTableModal = ({ onAcCodeClick,name, }) => {
  ...
  const [enteredGstStateCode, setenteredGstStateCode] = useState("");
  ...
  const fetchAndOpenPopup = async () => {
    try {
      const response = await axios.get('http://localhost:5000/groupmaster/getgststatemasterhelper');
  ...
  const handleMillCodeButtonClick = () => {
    ...
    fetchAndOpenPopup();
    if (onAcCodeClick) {
      onAcCodeClick(enteredGstStateCode);
    }
  };
  ...
  const handleAcCodeChange = (event) => {
    ...
    const matchingItem = popupContent.find((item) => item.State_Code === parseInt(value, 10));

    if (matchingItem) {
      setEnteredAcName(matchingItem.State_Name);
      console.log("Ac_Name_E:", matchingItem.State_Name);
    }
  };
  ...
  const handleRecordDoubleClick = (item) => {
    if (lActiveInputFeild === name) {
      setenteredGstStateCode(item.State_Code);
      setEnteredAcName(item.State_Name);
      if (onAcCodeClick) {
        onAcCodeClick(item.State_Code);
      }
    } 
