...
const ApiDataTableModal = ({ onAcCodeClick,name, }) => {
  ...
  const [enteredGstRateCode, setenteredGstRateCode] = useState("");
  ...
  const fetchAndOpenPopup = async () => {
    try {
      const response = await axios.get('http://localhost:5000/groupmaster/gstratemasterhelper');
  ...
  const handleMillCodeButtonClick = () => {
    ...
    fetchAndOpenPopup();
    if (onAcCodeClick) {
      onAcCodeClick(enteredGstRateCode);
    }
  };
  ...
  const handleAcCodeChange = (event) => {
    ...
    if (matchingItem) {
      ...
      console.log("Ac_Name_E:", matchingItem.GST_Name);
    }
  };
  ...
  const handleRecordDoubleClick = (item) => {
    if (lActiveInputFeild === name) {
      setenteredGstRateCode(item.Doc_no);
