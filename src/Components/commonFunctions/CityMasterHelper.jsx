...
const ApiDataTableModal = ({ onAcCodeClick,name, }) => {
  ...
  const [enteredCityMasterCode, setenteredCityMasterCode] = useState("");
  ...
  const fetchAndOpenPopup = async () => {
    try {
      const response = await axios.get('http://localhost:5000/groupmaster/citymasterhelper');
  ...
  const handleMillCodeButtonClick = () => {
    ...
    fetchAndOpenPopup();
    if (onAcCodeClick) {
      onAcCodeClick(enteredCityMasterCode);
    }
  };
  ...
  const handleAcCodeChange = (event) => {
    ...
    setenteredCityMasterCode(value);
    ...
    const matchingItem = popupContent.find((item) => item.city_code === parseInt(value, 10));

    if (matchingItem) {
      setEnteredAcName(matchingItem.city_name_e);
      setEnteredAccoid(matchingItem.bsid);
      cityidfeild = matchingItem.cityid;
      console.log("mc code is:", cityidfeild);
      console.log("Ac_Name_E:", matchingItem.city_name_e);
    } else {
      setEnteredAcName("");
      setEnteredAccoid("");
    }
  ...
  const handleRecordDoubleClick = (item) => {
    if (lActiveInputFeild === name) {
      setenteredCityMasterCode(item.city_code);
      cityidfeild = item.cityid;
      console.log("mc code is:",cityidfeild);
      setEnteredAcName(item.city_name_e);
      if (onAcCodeClick) {
        onAcCodeClick(item.city_code);
      }
    } 
  ...
  const filteredData = popupContent.filter((item) =>
    ...
    item.city_name_r.toLowerCase().includes(searchTerm.toLowerCase())
  );
